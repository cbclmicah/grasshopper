...
var getUser = module.exports.getUser = function(id, callback) {
    DB.User.findById(id).complete(function(err, user) {
        if (err) {
...
var getUserByCredentials = module.exports.getUserByCredentials = function(appId, strategy, identifier, callback) {
    ...
    if (strategy === AuthConstants.strategies.LOCAL) {
        where['email'] = identifier;
    } else {
        where['shibbolethId'] = identifier;
    }

    DB.User.findOne({'where': where}).complete(function(err, user) {
        if (err) {
...
var createLocalUser = function(appId, userProfile, credentials, callback) {
    ...
    DB.User.create(localUser).complete(function(err, createdUser) {
        if (err) {
            log().error({'err': err, 'user': localUser}, 'Failed to create a new user with a local authentication strategy');
            return callback({'code': 500, 'msg': err.message});
        }

        log().debug({'createdUser': createdUser}, 'Created a new user with a local authentication strategy');
...
var createShibbolethUser = function(appId, userProfile, credentials, callback) {
    ...
    DB.User.create(shibbolethUser).complete(function(err, createdUser) {
        if (err) {
            log().error({'err': err, 'user': shibbolethUser}, 'Failed to create a new user with a Shibboleth authentication strategy');
            return callback({'code': 500, 'msg': err.message});
        }

        log().debug({'createdUser': createdUser}, 'Created a new user with a Shibboleth authentication strategy');
        return callback(null, createdUser);
    });
};
 ...
 *
 * @param  {User}       user                The lecturer to get the calendar for
 ...
 */
var getLecturerCalendar = module.exports.getLecturerCalendar = function(user, includeContext, start, end, callback) {
    var dateTimeFilter = {};
    if (start) {
    ...
    var options = {'model': DB.Event, 'where': dateTimeFilter, 'required': false, 'attributes': eventFields, 'include': [
			{'model': DB.User, 'as': 'Organisers', 'required': false, 'attributes': userFields}
		]
	};

    user.getOrganisedEvents(options).complete(function(err, events) {
        if (err) {
            log().error({'err': err, 'user': user.id}, 'Could not get a lecturer\'s calendar');
            return callback({'code': 500, 'msg': 'Could not get a lecturer\'s calendar'});
        }

        // If the user isn't listed as an organiser , then they won't have a calendar record yet. In that case, we simply return an empty array.
        if (!events) {
            return callback(null, []);
        }
...
/**
 * Get users by their shibboleth id
 *
 * @param  {Number}     appId               The id of the app to get the users for
 * @param  {String[]}   shibbolethIds       The shibboleth ids of the users to retrieve
 * @param  {Function}   callback            Standard callback function
 * @param  {Object}     callback.err        An error object, if any
 * @param  {User[]}     callback.users      The requested users
 */
var getUsersByShibbolethId = module.exports.getUsersByShibbolethId = function(appId, shibbolethIds, callback) {
    if (_.isEmpty(shibbolethIds)) {
        return callback(null, []);
    }
    ...
    var options = {
        'where': {
            'AppId': appId,
            'shibbolethId': shibbolethIds
        }
    };
    DB.User.findAll(options).complete(function(err, users) {
        if (err) {
            log().error({'err': err, 'shibbolethIds': shibbolethIds}, 'Unable to find users by their shibboleth id');
            return callback({'code': 500, 'msg': 'Unable to find users by their shibboleth id'});
        }
